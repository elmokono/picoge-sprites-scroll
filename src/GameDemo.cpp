...
Piece GameDemo::getCurrentPiece()
{
  return currentPiece;
}
...
void GameDemo::draw(void)
{
  ...
  engine_core_ref->canvas->print(4, 4, "LIVES x 999 - SCORE 999", MAGENTA);
...
int GameDemo::getScore()
{
  return score;
}

bool GameDemo::hasBlock(int x, int y)
{
  return gameMap[x][y];
}

void GameDemo::addPiece()
{
  // start position
  srand(time(NULL));
...
void GameDemo::movePiece(int direction)
{
...
bool GameDemo::checkPieceCollision()
{
  for (int i = 3; i > -1; i--)
...
void GameDemo::rotatePiece(bool cw)
{
